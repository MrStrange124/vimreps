...
export function ExerciseRunner({ exercises, onSolved, onFinished, label }: Props) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-rule bg-panel">
      <header className="flex items-center gap-3 border-b border-rule px-3 py-2 text-[0.75rem] text-muted">
        <span className="text-faint">{label ?? "exercise"}</span>
        <span className="text-ink">
          {index + 1}/{exercises.length}
        </span>
        <div className="ml-auto flex items-center gap-2">
          {/* One way only. Once you have read a hint there is nothing to gain by
              putting it away again, and a toggle just asks a question that has
              no interesting answer. */}
          {exercise.hint && !showHint && (
            <button
              type="button"
              onClick={() => setShowHint(true)}
              className="rounded border border-rule px-2 py-0.5 text-[0.6875rem] text-muted hover:text-ink"
            >
              hint
            </button>
          )}
      ...
      <p className="px-4 pt-3 pb-2 text-[0.84375rem] text-ink">{exercise.prompt}</p>

      {showHint && exercise.hint && (
        <p className="mx-4 mb-2 rounded border border-rule bg-raised px-3 py-2 text-[0.78125rem] text-muted">
          {exercise.hint}
        </p>
      )}
      ...
      <footer className="flex min-h-[2.625rem] items-center gap-3 px-4 py-2 text-[0.78125rem]">
        {attempt.message ? (
        ...
        {attempt.status === "passed" && (
          <button
            type="button"
            onClick={advance}
            className="ml-auto rounded bg-accent px-3 py-1 text-[0.75rem] font-semibold text-ground hover:brightness-110"
          >
        ...
        {attempt.status === "failed" && (
          <button
            type="button"
            onClick={reset}
            className="ml-auto rounded border border-rule px-3 py-1 text-[0.75rem] text-ink hover:bg-raised"
          >
            Try again ⏎
          </button>
        )}
