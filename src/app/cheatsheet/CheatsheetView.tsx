...
export function CheatsheetView() {
  ...
  return (
    <main className="mx-auto w-full max-w-4xl flex-1 px-5 py-9">
      <h1 className="font-sans text-[1.75rem] font-semibold text-white">Cheatsheet</h1>
      ...
      <label className="mb-6 block">
        ...
        <input
          ...
          className="w-full rounded border border-rule bg-panel px-3 py-2 text-[0.8125rem] text-ink placeholder:text-faint focus:border-accent-dim focus:outline-none"
        />
      </label>
          ...
          <div className="divide-y divide-rule rounded border border-rule">
            {rows.map((row, index) => (
              <div
                ...
                className="flex flex-wrap items-baseline gap-x-4 gap-y-1 px-3 py-2 text-[0.8125rem]"
              >
                ...
                <Link
                  ...
                  className="text-[0.71875rem] text-faint hover:text-ink"
                >
                  {row.lessonTitle}
                </Link>
              </div>
